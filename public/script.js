...
async function openProjectModal(projectId) {
    try {
        ...
        modalBody.innerHTML = `
            ...
            ${goalMet && project.status === 'funded' && window.ethereum && metaMaskWallet.isConnected ? `
                <div style="margin-top: 2rem; padding: 1rem; background: #d4edda; border-radius: 8px; border: 1px solid #c3e6cb;">
                    <h4 style="color: #155724; margin-top: 0;">Goal Reached!</h4>
                    <p style="color: #155724; margin-bottom: 1rem;">This campaign has reached its funding goal. The creator can now withdraw the funds.</p>
                    <button onclick="withdrawFunds(${project.id}, '${project.creator_address}', ${project.current_amount})" class="btn btn-success">
                        Withdraw Funds (${project.current_amount} ETH)
                    </button>
                </div>
            ` : ''}

            ${isExpired && !goalMet && project.status !== 'refunded' && project.contributions && project.contributions.length > 0 ? `
                <div style="margin-top: 2rem; padding: 1rem; background: #f8d7da; border-radius: 8px; border: 1px solid #f5c6cb;">
                    <h4 style="color: #721c24; margin-top: 0;">Campaign Expired</h4>
                    <p style="color: #721c24; margin-bottom: 1rem;">This campaign has expired without reaching its goal. All contributors will be automatically refunded.</p>
                    <button onclick="triggerAutoRefund(${project.id})" class="btn" style="background: #dc3545; color: white;">
                        Process Auto-Refund
                    </button>
                </div>
            ` : ''}
            ...
            ${project.status === 'withdrawn' ? `
                <div style="margin-top: 2rem; padding: 1rem; background: #d1ecf1; border-radius: 8px; border: 1px solid #bee5eb;">
                    <p style="color: #0c5460; margin: 0;">Funds have been withdrawn by the creator.</p>
                </div>
            ` : ''}
            ...
            ${project.status === 'refunded' ? `
                <div style="margin-top: 2rem; padding: 1rem; background: #fff3cd; border-radius: 8px; border: 1px solid #ffeaa7;">
                    <p style="color: #856404; margin: 0;">All funds have been refunded to contributors.</p>
                </div>
            ` : ''}
        `;
